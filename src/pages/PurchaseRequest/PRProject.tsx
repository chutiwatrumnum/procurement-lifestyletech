...
export default function PRProject() {
  ...
  const handleSubmit = async (status: string) => {
    ...
    try {
      ...
      console.log('[PRProject] user.role:', user?.role, '| isManagerRole:', isManagerRole, '| status:', status, '| finalStatus:', finalStatus);
      ...
      if (isEditMode && id) {
        ...
        toast.success(status === 'draft' ? 'บันทึกร่างเรียบร้อย' : (isManagerRole && status === 'pending' ? 'อนุมัติใบขอซื้อเรียบร้อยแล้ว' : 'ส่งใบขอซื้อเรียบร้อยแล้ว'));
      } else {
        // Create new PR
        const prData: any = {
          type: 'project',
        ...
        // ส่ง notification เมื่อส่ง PR ใหม่ (ไม่ส่งถ้า manager อนุมัติเองแล้ว)
        if (status === 'pending' && !isManagerRole) {
          try {
        ...
        toast.success(status === 'draft' ? 'บันทึกร่างเรียบร้อย' : (isManagerRole ? 'อนุมัติใบขอซื้อเรียบร้อยแล้ว' : 'ส่งใบขอซื้อเรียบร้อยแล้ว'));
      }
      
      if (status === 'pending' && !isManagerRole) {
        // พนักงานทั่วไป → ไปหน้า approval
